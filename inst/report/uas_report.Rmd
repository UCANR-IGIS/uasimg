...
function set_src(elmnt) {
  ...
  elmnt.parentNode.parentNode.getElementsByClassName("fn")[0].textContent = elmnt.textContent;
  elmnt.parentNode.parentNode.getElementsByClassName("dt")[0].textContent = elmnt.getAttribute("data-dt");
}
```
...
cat("<tr><td>__Image size__:</td><td><span id='img_dim'>", img_pixels, "</span></td></tr>", sep = "")
...
if (group_img) {
  ...
  if (params$thumbnails) {
      ...
      summarise(count = n(), first_dt = first(pts_dt),
                balloon_html = paste0("<div><p>",
                                      paste("<span onclick='set_src(this)' data-tbfn='", tb_fn,
                                            "' data-dt='",
                                            format(pts_dt, "%Y-%m-%d %H:%M:%S"), "' class='btext'>",
                                            file_name, "</span>", sep = "", collapse = " "),
                                      "</p>",
                                      "<table class='fn_dt'><tr><td class='fn'>", first(file_name), "</td><td class='dt'>",
                                      format(first(pts_dt), "%Y-%m-%d %H:%M:%S"), "</td></tr></table>",
                                      "<img src='./tb/",
                                      first(tb_fn), "' width='440' height='300' class='bln_img'/></div>")) %>%
    arrange(first_dt)
    ...
    leaf_pts_tbl <- leaf_pts_tbl %>% 
      summarise(count = n(), first_dt = first(pts_dt),
                balloon_html = paste(file_name, "<br/>", sep = "", collapse = "<br/>")) %>% 
    arrange(first_dt)
  }
  ...
  #                                 paste("<span onclick='set_src(this)' data-tbfn='", tb_fn, 
  #                                       "' data-dt='", 
  #                                       format(pts_dt, "%b %d, %Y. %I:%M:%S %p"), "' class='btext'>", 
  #                                       file_name, "</span>", sep = "", collapse = " "), 
  #                         "</p>",
  #                         "<table class='fn_dt'><tr><td class='fn'>", first(file_name), "</td><td class='dt'>", 
  #                         format(first(pts_dt), "%b %d, %Y. %I:%M:%S %p"), "</td></tr></table>", 
  #                         "<img src='./tb/", 
  #                         first(tb_fn), "' width='440' height='300' class='bln_img'/></div>"),
  #           ) %>% 
  #   arrange(first_dt)
