...
#'
#' If your objective is to open the images in ArcGIS or QGIS, then generating
#' the \code{aux.xml} files should be all you need. \code{aux.xml} files
#' are \href{http://desktop.arcgis.com/en/arcmap/latest/manage-data/raster-and-images/auxiliary-files.htm}{ESRI auxillary files}
#' for raster layers. To be read by GIS software, they should have the same name as
#' the image file with 'aux.xml' added on. aux.xml files can contain a lot of info
#' about a raster layer, including statistics, the rotation info, coordinate
#' reference system, and other stuff. If generated by this function, they will
#' contain projection and rotation info only. Note also \strong{any existing aux.xml
#' files will be overwritten}. aux.xml files are the only sidecar file that ArcGIS
#' software supports for reading the projection info.
#'
