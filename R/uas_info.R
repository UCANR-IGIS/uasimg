...
#' To index a specific file type, you can pass one or more file extensions to \code{ext} (not case sensitive).
#'
#' If you are a Windows user and the output includes \code{-- press ENTER --} statements, you can ignore them.
#' These are generated by the exiftool executable. To get rid of them, find the exiftool executable (probably
#' in tools::R_user_dir("exiftoolr")) and rename the 'exiftool(-k).exe' file to 'exiftool.exe' (if you don't
#' see the '.exe.' extension in Windows Explorer, you probably have file extension hidden).
#'
...
uas_info <- function(img_dirs, ext = c("jpg", "jpeg", "tif", "tiff", "raw", "dng")[1:4], alt_agl = NULL, fp = FALSE, fwd_overlap = fp,
  ...
  if (is.null(ext)) {
    ext_cmdarg <- ""
    ext_regex_chr <- ".jpg$|.jpeg$|.tif$|.tiff$|.raw$|.dng$"
  ...
  for (img_dir in img_dirs) {
    ...
    if (!is.null(cache)) {
      ...
      if (!is.na(cache_dir_use)) {
        ...
        dir_files <- dir_files[grepl(ext_regex_chr, dir_files, ignore.case=TRUE)]
    ...
    if (!cache_loaded) {
      ...
      first_fn <- list.files(path = img_dir, full.names = TRUE, pattern = ext_regex_chr, ignore.case = TRUE)[1]

      if (is.na(first_fn)) stop(paste0("Couldn't find any image files that have the extension in ", img_dir))
