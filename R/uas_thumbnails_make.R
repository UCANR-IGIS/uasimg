...
#' @param img_dir Image folders in x to process (NULL to process all)
...
uas_thumbnails_make <- function(x, img_dir = NULL, output_dir = NULL, tb_width = 400,
    ...
    for (idir in dirs_use) {
        ...
        if (!is.null(output_dir)) {
            ...
            output_dir_use <- file.path(idir, "map", "tb")
        ...
        if (is.na(null2na(x[[idir]]$metadata$name_short))) {
            ...
            flight_name <- x[[idir]]$metadata$name_short
        }
